...
def get_rnn_dataloaders(encoded_data, dataset_type, iterator='', sess_length = 20, feature_width = 28):
    ...
    """
    rnn_dir="../data/for_direct_rnn/" # run outside env
    #rnn_dir="for_rnn/" #run inside env

    encoded_data=pd.read_csv("../data/for_encoder/"+encoded_data+"_"+dataset_type+iterator+" - Copy.csv", header=None).to_numpy()

    print(encoded_data.shape)

    encoded_data = torch.tensor(encoded_data, dtype=torch.float)#.to('cuda:0')
...
def read_rnn_dataloaders(features, labels, dataset_type, iterator='', batch_size=1):
    """
    inputs:
    ...
    labels=encoded_dir+labels+"_"+dataset_type+iterator+" - Copy.csv"
    ...
    dataset = torch.utils.data.TensorDataset(
        features.to('cuda:0'),#.double(),
        labels.to('cuda:0')#.double()
        )
...
#get_rnn_dataloaders('features', 'train', '1')
#get_rnn_dataloaders('features', 'test', '1')
#get_rnn_dataloaders('features', 'valid', '1')